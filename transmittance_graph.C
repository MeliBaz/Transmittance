...
cout<<"First "<<(1/N)*pow(mean,(1-N)/N)*exp(-l/(N*L))*mean_error<<endl;
cout<<"Second "<<-pow(mean,1/N)*exp(-l/(L*N))*delta_l/(L*N) <<endl;
cout<<pow((1/N)*pow(mean,(1-N)/N)*exp(-l/(N*L))*mean_error,2)+pow(-pow(mean,1/N)*exp(-l/(L*N))*delta_l/(L*N),2)<<endl;
